...
\begin{definition}(Vektorraum)  \\
	...
	Ein Tripel $(V,+,\cdot)$, bestehend aus einer Menge $V$, einer Abbildung (genannt Addition) \\	
		$+ : V \times V \rightarrow V$ \\
		    ...
		    $(1)\hspace{10pt}  (x + y) + y = x + (y + z) \hspace{10pt}\forall x,y,z \in V$ \\
		    $(2)\hspace{10pt} x + y = y + x \hspace{70pt}\forall x,y,z \in V$ \\
		    $(3)$\hspace{10pt}  Es gibt ein Element $0 \in V$ (genannt \glqq Null{} \grqq oder\glqq  Nullvektor \grqq ) mit\\
		    \hspace{10pt} $x + 0 = x \hspace{116pt} \forall x \in V$ \\
		    $(4)\hspace{10pt}$ Zu jedem $x \in V$ gibt es  ein Element $-x \in V$ mit $x + (-x) = 0 $ \\
		    $(5)\hspace{10pt}  \lambda (\mu x) = (\lambda \mu) x \hspace{67pt}\forall \lambda, \mu \in \mathbb{R}, x\in V$ \\
		    $(6)\hspace{10pt}  1x = x \hspace{108pt} \forall x \in V$  \\
		    $(7)\hspace{10pt}  \lambda (x + y) =  \lambda x + \lambda y \hspace{43pt}\forall \lambda \in \mathbb{R}, x,y \in V$ \\
		    $(8)\hspace{10pt}  (\lambda + \mu)x =  \lambda x + \mu x \hspace{43pt}\forall \lambda, \mu \in \mathbb{R}, x \in V$ \\
...
Sei $D = {d_1,...,d_n}$ eine Menge von Dokumenten oder Objekten und $A = {A_1,...,A_n}$ eine Menge von Attributen $A_j: D \rightarrow R$ auf diesen Objekten. Die Attributwerte $A_j(d_i) =:w_i,_j$ des Dokuments $d_i$ lassen sich als Gewichte auffassen und zu einem Vektor $w_i = (w_i,_1,...,w_i,_n) \in \mathbb{R}^{n}$ zusammenfassen. Dieser Vektor beschreibt das Dokument im Vektorraummodell: Er ist seine Repräsentation und wird Dokumentvektor genannt. \\
