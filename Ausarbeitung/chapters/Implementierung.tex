...
Außerdem muss dem Nutzer durch Ranking angezeigt werden, wie hoch der Ähnlichkeitsgrad der einzelnen Dokumente ist, um wichtige von weniger relevanten Treffern differenzieren zu können. \\
 ...
 Für das Vektorraummodell werden gleich mehrere Hash-Tables benötigt:
 \begin{enumerate}
 	\item Verwaltung des Vokabulars
 	\item 
 \end{enumerate}
...
So kann für jeden Term des Vokabulars schnell dessen Index abgefragt werden. Dies wird später für die Erstellung der Vektoren notwendig sein, da der Index die Postion im Vektor bestimmt. 
 ...
 Zur Bestimmung der Dokumentvektoren werden die idf-Werte benötigt (\ref{idf}). Hierzu werden bereits während der Indexierung die Dokumenthäufigkeiten bestimmt und diese neben dem Index in derselben Hash-Table gespeichert. Hier wurde erneut ein Struct zum Zusammenfassen der Inhalte verwendet. \\
 ...
 Würde man die Dokumentvektoren vollständig anlegen, d.h. mit einem Eintrag pro Term, ergäbe sich eine Matrix mit den \textit{docID}s als Zeilen und den\textit{ Termindizes} als Spalten. Dies würde zum gleichen Problem wie bei einer Term-Inzidenz-Matrix führen: Viele Gewichte sind 0, weil die Terme gar nicht vorkommen, was zu viel Speicher verbraucht. \\
Stattdessen wurden auch für das Vektorraummodell invertierte Listen verwendet: Nur die Einträge ungleich null werden gespeichert. \\
Hierzu wird, wie auch schon bei den modifizierten invertierten Listen für die Keywordsuche, eine Hash-Table verwendet.


\subsubsection{Berechnung des Cosinus-Maßes}
Da alle Vektoren in 
