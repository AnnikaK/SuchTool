...
In diesem Kapitel wird beschrieben, auf welche Weise das System zur Informationssuche in einem Dokumentenarchiv basierend auf Textinhalt sowie Metadaten in der Programmiersprache Lisp realisiert wurde.
...
\begin{figure} [http]
	...
	\caption{Eintrag im Dokument-Dictionary. Die Struct-Slots Datum und Absender können, falls nicht existent, auch leer bleiben. Im rot markierten Bereich wird später der Dokumentvektor eingetragen (Eigene Abbildung).}
	\label{docs}
	
\end{figure}

\subsection{Verarbeiten der Dokumente}
Anschließend wird über alle Dokumente im Dokument-Dictionary iteriert, um die folgenden Schritte auszuführen:
\begin{enumerate}
...
\begin{figure} [http]
	...
	\caption{Modifizierte invertierte Liste zur Realisierung der Metadatensuche. Statt der \textit{docID}s, wie es bei invertierten Listen üblich ist, werden Structs der Form (docID, Typ, Inhalt) gespeichert(Eigene Abbildung).}
	\label{m}
...
Lautet die Anfrage beispielsweise \textit{Absender=\glqq Klaus\grqq{}}, wird auf die modifizierte invertierte Liste über den Hash-Key \textit{Absender} zugegriffen. Anschließend wird über alle darin gespeicherten Structs iteriert, wobei zunächst der Typ des Inhalts abgefragt wird. Dieser entscheidet über die Art der Suche:
\begin{enumerate}
